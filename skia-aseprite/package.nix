{ aseprite, clangStdenv, expat, fetchFromGitHub, fetchgit, fontconfig, gn
, harfbuzzFull, icu, lib, libglvnd, libjpeg, libpng, libwebp, libX11, libgbm
, ninja, python3, zlib, xcbuild }:
...
in clangStdenv.mkDerivation (finalAttrs: {
  ...
  nativeBuildInputs = [ gn ninja python3 xcbuild ];
  ...
  buildInputs =
    [ expat fontconfig harfbuzzFull icu libjpeg libpng libwebp zlib ]
    ++ (lib.optionals clangStdenv.isLinux [ libglvnd libX11 libgbm ]);

  buildPhase = ''
    runHook preBuild
    # DIRTY DARWIN HACK: add toolchain to our PATH for the correct libtool
    export PATH="$(xcode-select --print-path)/Toolchains/XcodeDefault.xctoolchain/usr/bin:$PATH"
  ...
  meta = {
    description =
      "Complete 2D graphic library for drawing Text, Geometries, and Images (Aseprite's fork)";
